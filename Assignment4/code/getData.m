...
switch whatdata
    ...
    case {'left', 'left-plot', 'right', 'right-plot'}
        ...
        data.im = im;
        try
            car = strcat(imname,'detector-car');
            data.car = load(car);
            data.car_name = car;
        catch
            data.car_name = car;
        ...
        try
            person = strcat(imname,'detector-person');
            data.person = load(person);
        ...
        try
           cyclist = strcat(imname,'detector-cyclist');
           data.cyclist = load(cyclist);
           data.cyclist_name = cyclist;
        catch
           data.cyclist_name = cyclist;
        end
        ...
        try
            inbox = strcat(imname,'_inbox');
            data.inbox = load(inbox);
        catch
            data.inbox = 0;
        end
